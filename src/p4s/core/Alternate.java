...
public class Alternate extends AlternateDataStructure implements CDProtocol, EDProtocol {
    ...
    private static Alternate receiver;
//--------------------------------------------------------------------------
// Initialization
//--------------------------------------------------------------------------
            ...
            if (sender.getActiveUp(node) >= sender.getActiveUpload(node)) {//Sender has reached #max of active upload connections
                ...
                while (sender.getActiveUp(node) < sender.getActiveUpload(node) && sender.getPushAttempt() <= sender.getPushRetry() && sender.getCycle() == Message.PUSH_CYCLE) {
                    ...
                    if (peer != null) {//there exist a target candidate
                        ...
                        delay = this.send(node, peer, imm, pid);
                        if (sender.getDebug() >= 2) {
                            System.out.println("\t\t" + sender.getActiveUp(node) + ") Node " + node.getID() + " PUSHes " + imm.getChunkids() + " to Node " + peer.getID() + " MexRX " + (CommonState.getTime() + delay) + " > (" + delay + ")");
                        }
                    } else {
        ...
        if (receiver.getDownload(node) <= receiver.getDownloadMin(node)) {
            ...
            while (receiver.getActiveDw(node) < receiver.getActiveDownload(node) && receiver.getPullAttempt() < receiver.getPullRetry() && receiver.getCycle() == Message.PULL_CYCLE) {
                ...
                if (peer != null) {
                    ...
                    if (receiver.getDebug() >= 2) {
                        System.out.println("\t\tNode " + node.getID() + " PULLes " +
                                imm.getChunkids() + "to Node " + peer.getID() + " MexRX " + (delay + CommonState.getTime()) + " >(" + delay + ")");
                    }
                } else {
    ...
    public void processEvent(Node node, int pid, Object event) {
        ...
        switch (im.getMessage()) {
            case Message.SWITCH_PUSH: {
                this.exSwitchPush(node, pid, im);
                break;
            }
            case Message.PUSH: {
                //************************************* N O D E   R E C E I V E S   P U S H   ******************************************
                this.exPush(node, pid, im);
                break;
            }
            case Message.OK_PUSH: {
                //************************************* N O D E   R I C E V E S   O K    F O R    P U S H ******************************************                
                this.exOkPush(node, pid, im);
                break;
            }
            case Message.START_PUSH: {
                //************************************* N O D E   R I C E V E S   S T A R T    P U S H ******************************************
                this.exStartPush(node, pid, im);
                break;
            }
            case Message.FINISH_PUSH: {
                //************************************* N O D E   R I C E V E S   F I N I S H    P U S H ******************************************
                this.exFinishPush(node, pid, im);
                break;
            }
            case Message.NO_CHUNK_OWNED: {
                //************************************* N O D E   R I C E V E S   N O    C H U N K    O W N E D ******************************************
                this.exNoChunkOwned(node, pid, im);
                break;
            }
            ...
            case Message.PULL: {
                //************************************* N O D E   R E C E I V E S   A   P U L  L ******************************************
                this.exPull(node, pid, im);
                break;
            }
            case Message.OK_PULL: {
                //************************************* N O D E   R E C E I V E S   O K    P U L  L ******************************************
                this.exOkPull(node, pid, im);
                break;
            }
            case Message.START_PULL: {
                //************************************* N O D E   S T A R T    T H E    P U L  L ******************************************
                this.exStartPull(node, pid, im);
                break;

            }
            case Message.FINISH_PULL: {
                //************************************* N O D E   R E C E I V E S   F I N I S H   P U L  L ******************************************
                this.exFinishPull(node, pid, im);
                break;
            }
            case Message.NO_CHUNK_UNAVAILABLE: {
                //************************************* N O D E   R E C E I V E S   N O   C H U N K S ******************************************
                this.exNoChunkUnavailable(node, pid, im);
                break;
            }
            case Message.IN_PULLING: {
                //************************************* N O D E   R E C E I V E S   I N   P U L L I N G  ******************************************
                this.exInPulling(node, pid, im);
                break;
            }
            case Message.NO_UPLOAD_BANDWIDTH_PULL: {
                //************************************* N O D E   R E C E I V E S   N O   U P L O A D   T O   P U L  L ******************************************
                this.exNoUploadBandwidthPull(node, pid, im);
                break;
            }
            default: {
